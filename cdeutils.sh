...
subcommand_usage() {
    ...
    Sub-commands:
                ...
                init-virtual-cluster    Initialize a CDE Virtual Cluster            -h <virtual-cluster-host> [-a('auto-generate-certs')]
                                                                                    -h <virtual-cluster-host> -c <certificate-path> -k <keyfile-path>
        init-user-in-virtual-cluster    Initialize a user in a CDE Virtual Cluster  -h <virtual-cluster-host> -u <workload-user> -p <principal-file> -k <keytab-file>" 1>&2
}
...
__setup_ingress() {
    ...
    if [ x"${rc}"x != x""x ]; then
         log_info "Ingress ${INGRESS_PREFIX}-api patched already in ${K8S_NAMESPACE} namespace, not doing anything and returning ..."
         return
    fi
